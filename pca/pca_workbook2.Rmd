...
Height in the roster data is a character in a bad format e.g. "6-4", so I wrote a function to convert it to cm.
...
qb_roster <- rosters %>%
  ...
  mutate(
    ...
    age = format(Sys.time(), "%Y") %>% as.numeric() - lubridate::year(birth_date)
  ) %>%
  select(-birth_date)

qbs <- qb %>% 
  filter(passer_id %in% qb_roster[['passer_id']] | rusher_id %in% qb_roster[['passer_id']]) %>%
  # doing this now so I can make a label with the season and the QB name
  left_join(select(qb_roster, full_name, passer_id = passer_id)) %>%
  mutate(name_label = paste0(full_name, ", ", season))
```
...
qb_rushes <- filter(qbs, play_type == 'run') %>%
  ...
  left_join(select(qb_roster, -c(position, headshot_url)))
...
# dropped from 145 to 136 players
qb_passes <- qb_passes %>%
