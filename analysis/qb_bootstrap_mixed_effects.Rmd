...
pbp_mut <- readRDS("~/Downloads/sort_newest/r/archives_sort_this/Mixed_Effects_with_Bootstrapping_Tutorial-master/data/pbp_mut.RDS")
```
...
resampler <- function(dat, clustervar, replace = TRUE, reps = 1) {
  ...
  dat$Replicate <- factor(cut(dat$NewID,
    breaks = c(1, ncid * 1:reps), include.lowest = TRUE,
    labels = FALSE
  ))
...
```{r}
start_time <- Sys.time()
indx <- resampler(pbp_mut, "player_id", reps = 200)
end_time <- Sys.time()
end_time - start_time

start_time <- Sys.time()
resampled_data <- cbind(indx, pbp_mut[indx$RowID, ])
end_time <- Sys.time()
end_time - start_time
```
...
```{r}
start <- proc.time()
output <- parLapplyLB(clus, X = levels(resampled_data$Replicate), fun = boot_function)
end <- proc.time()
end - start
...
```{r}
coach <- final_transposed %>%
  select(pos_coach..Intercept.) %>%
  mutate(Variable = "Head Coach") %>%
  select(intercept = pos_coach..Intercept., Variable)

qb <- final_transposed %>%
  select(player_id..Intercept.) %>%
  mutate(Variable = "Quarterback") %>%
  select(intercept = player_id..Intercept., Variable)

def_coach <- final_transposed %>%
  select(def_coach..Intercept.) %>%
  mutate(Variable = "Opposing Head Coach") %>%
  select(intercept = def_coach..Intercept., Variable)

team <- final_transposed %>%
  select(team..Intercept.) %>%
  mutate(Variable = "Team (Proxy for Offensive Supporting Cast)") %>%
  select(intercept = team..Intercept., Variable)
...
plot %>%
  ...
  labs(
    x = "Absolute Impact on Passing Offense Efficiency (EPA) | Further to the right means greater impact (positive or negative)", y = "Density",
...
Mixed Effects Model to Find Intercept | Using 200 bootstrapped (re-sampled) datasets to find the distribution of Cholesky Factorized Coefficients"
  ...
  annotate(
    geom = "label", x = .001, y = 135, hjust = "left", fill = "#F0F0F0", vjust = 1,
...
without an elite QB or a good QB + supporting cast combination",
  ...
  annotate(
    geom = "label", x = .095, y = 65, hjust = "left", size = 1.45, family = "Trebuchet MS", color = "grey20", fill = "#F0F0F0", vjust = 1,
...
and upside"
  ...
  annotate(
    geom = "text", x = .101, y = 115, hjust = "left", size = 1.45, family = "Trebuchet MS", color = "grey20",
...
efficiency"
  ...
  annotate(
    geom = "text", x = .051, y = 92, hjust = "left", size = 1.45, family = "Trebuchet MS", color = "grey20",
...
more than the QB"
  ...
  annotate(
    geom = "text", x = .001, y = 77, hjust = "left", size = 1.45, family = "Trebuchet MS", color = "grey20",
