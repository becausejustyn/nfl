...
```
#https://www.sportsbook.football/calculating-hfa-in-the-nfl-with-r-and-the-tidyverse/
```
...
```{r}
library(tidyverse)
nfl <- read_csv("~/Documents/Github/nfl/workbooks/GAME.csv")
...
summarise(nfl, diff = sum(ptsh - ptsv) / length(ptsh))
```
...
hfa_by_team <- nfl %>%
  ...
  mutate(
    mov_spread = round(h, 2) - round(v, 2),
